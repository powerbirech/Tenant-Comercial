...
for key, value in config_environments:
    ...
    connection_string = semanticmodel_parameters.get("SqlConnectionString", None)
  ...
  #  )
    ...
    semanticmodel_id = deploy_item(
        ...
        find_and_replace={
            (
                r".*\.tmdl",
                r'Odbc\.DataSource\("dsn=.*?"',
            ): f'Odbc.DataSource("{connection_string}"',
            (
                r".*\.tmdl",
                r'(\[Name=")([^"]+)",Kind="Database"\]',
            ): rf'\1{database}",Kind="Database"]',
        },
    )
